...
struct DisplayTextStyle final {
  std::uint16_t font_id{0U};
  std::uint16_t font_size_px{10U};
  bool emphasized{false};
};

struct DisplayTheme final {
  DisplayTextStyle title{0U, 15U, true};
  DisplayTextStyle warning{0U, 15U, true};
  DisplayTextStyle label{0U, 10U, false};
  DisplayTextStyle value{0U, 10U, false};
};

struct DisplayTextElement final {
...
inline bool displayThemeValid(const DisplayTheme& theme) noexcept {
  return theme.title.font_size_px > 0U && theme.warning.font_size_px > 0U &&
         theme.label.font_size_px > 0U && theme.value.font_size_px > 0U;
}

inline const DisplayTextStyle& displayStyleForRole(DisplayTextRole role,
                                                   const DisplayTheme& theme) noexcept {
  switch (role) {
...
inline bool displayCommandFits(const DisplayTextCommand& command,
...
// Narrow display-render seam. A concrete display backend implements a sink with:
//   bool beginFrame(width_px, height_px, warning)
//   bool drawText(const DisplayTextElement&)
//   bool endFrame()
//   void cancelFrame()
...
bool renderDisplayList(const DisplayRenderList& render_list, const DisplayRenderGeometry& geometry,
                       const DisplayTheme& theme, ClaySink& sink) noexcept {
  if (render_list.command_count == 0U || render_list.command_count > render_list.commands.size() ||
      !detail::displayThemeValid(theme) || geometry.width_px == 0U || geometry.height_px == 0U) {
    return false;
  }

  for (std::size_t index = 0U; index < render_list.command_count; ++index) {
    if (!detail::displayCommandFits(render_list.commands[index], geometry)) {
      return false;
    }
  }
  ...
  for (std::size_t index = 0U; index < render_list.command_count; ++index) {
    ...
    const DisplayTextElement element{
        command.x_px,         command.y_px,
        command.max_width_px, detail::displayStyleForRole(command.role, theme),
        command.text.data(),
    };
