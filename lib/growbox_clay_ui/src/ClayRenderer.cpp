...
inline bool isColorDark(const Clay_Color& color) {
  const int sum = static_cast<int>(color.r) + static_cast<int>(color.g) + static_cast<int>(color.b);
  return sum < 3 * 128;
}
...
ClayRenderer::ClayRenderer(ClayRenderTarget& target) : _target(target) {
  _clipStack.reset();
}

void ClayRenderer::render(Clay_RenderCommandArray commands, const ClayRenderConfig& config) {
  (void)config;
  _clipStack.reset();
...
void ClayRenderer::handleBorder(const Clay_RenderCommand& command) {
  ...
  if (border.width.left > 0 && border.width.left == border.width.right &&
      border.width.left == border.width.top && border.width.left == border.width.bottom) {
...
void ClayRenderer::handleText(const Clay_RenderCommand& command) {
  ...
  _target.drawText(text.stringContents.chars, static_cast<int16_t>(text.stringContents.length),
                   static_cast<int16_t>(command.boundingBox.x),
                   static_cast<int16_t>(command.boundingBox.y), color, text.fontId, text.fontSize,
                   static_cast<int16_t>(command.boundingBox.width),
                   static_cast<int16_t>(command.boundingBox.height), clipX, clipY, clipW, clipH);
}
