...
Display UI Port Phases 4-5 are integrated. Phase 5 merged as `7533f7f4c2c92675ef0908c91912b89a66f7ee4a`: growbox-owned native ESP-IDF buttons use stable-edge debounce and semantic events while `DisplayRuntimeController` remains the navigation owner. Exact-SHA boot qualification on `/dev/cu.usbserial-120` confirmed `eink_ready=1` and `eink_buttons_ready=1` with outputs fake-locked. Manual physical key actuation remains a Phase 7 qualification item. Current next display work is Phase 6 in `docs/DISPLAY_UI_PORT.md`: only justified growbox menu/settings mechanics through existing application/domain APIs.
